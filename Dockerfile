...
ENV VIRTUAL_ENV /opt/neo-ls-svm-env
ENV PATH $VIRTUAL_ENV/bin:$PATH
RUN python -m venv $VIRTUAL_ENV

# Set the working directory.
WORKDIR /workspaces/neo-ls-svm/
...
COPY --chown=user:user poetry.lock* pyproject.toml /workspaces/neo-ls-svm/
RUN mkdir -p /home/user/.cache/pypoetry/ && mkdir -p /home/user/.config/pypoetry/ && \
    mkdir -p src/neo_ls_svm/ && touch src/neo_ls_svm/__init__.py && touch README.md
...
COPY --chown=user:user .pre-commit-config.yaml /workspaces/neo-ls-svm/
